...
function parseMarkdown(text, scope) {
  ...
  lines.forEach((line, idx) => {
    ...
    if (cbX || cbO) {
      if (inUl) { html += '</ul>'; inUl = false; } if (!inCb) { html += '<ul class="cb-list">'; inCb = true; }
...
function renderWeekPage() {
  const wk = state.activeWeek, journal = state.weekJournals[wk] || '', weekTasks = state.tasks.filter(t => t.weekKey === wk);
  const nowKey = getWeekKey(new Date());
  const isNow = wk === nowKey;
  const isFuture = wk > nowKey;
  ...
  return `<div class="page">
    <div class="page-header">
      <div class="week-nav">
        ...
        <span class="week-nav-label" onclick="openWeekPicker()">${getWeekLabel(wk)}</span>
        <button class="btn btn-sm btn-ghost btn-icon" onclick="shiftWeek(1)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg></button>
        ...
        ${isFuture ? `<span style="font-size:11.5px;color:var(--blue);background:var(--blue-light);padding:3px 10px;border-radius:20px;font-weight:500">Future week</span>` : ''}
      </div>
...
function openWeekPicker() {
  ...
  const todayKey = getWeekKey(now);
  const activeKey = state.activeWeek;

  // Generate weeks for the current year
  const weeks = [];
  let d = new Date(currentYear, 0, 4); // Jan 4 is always in week 1
  d.setDate(d.getDate() - d.getDay() + 1); // Start of week 1

  for (let i = 0; i < 53; i++) {
    const wk = getWeekKey(d);
    if (new Date(wk).getFullYear() > currentYear && i > 40) break;
    weeks.push({
      key: wk,
      num: getWeekNumber(wk),
      label: getWeekLabel(wk),
      isToday: wk === todayKey,
      isActive: wk === activeKey,
      hasContent: !!(state.weekJournals[wk]?.trim() || state.tasks.some(t => t.weekKey === wk))
    });
    ...
    <div class="modal-bg" onclick="closeModal()">
      <div class="modal" onclick="event.stopPropagation()">
        <div class="modal-header">
          <div class="modal-title">Select Week (${currentYear})</div>
          <button class="drawer-close" onclick="closeModal()">✕</button>
        </div>
        <div class="modal-body">
          <div class="week-grid">
            ${weeks.map(w => `
              <div class="week-btn${w.isActive ? ' active' : ''}${w.isToday ? ' today' : ''}${w.hasContent ? ' has-content' : ''}" 
                   onclick="selectWeek('${w.key}')" 
                   title="${w.label}">
                <div class="week-num">W${w.num}</div>
                <div class="week-date-hint">${new Date(w.key).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</div>
              </div>
            `).join('')}
          </div>
        </div>
  ...
  return `<div class="page">
    ...
    ${_wLoading ? `<div class="ai-generating"><div class="ai-dots"><div class="ai-dot"></div><div class="ai-dot"></div><div class="ai-dot"></div></div><span>Designing your personalised workout split…</span></div>`
      : plan ? plan.days.map((day, idx) => {
        const isRest = day.type === 'Rest', exp = _wExpanded[idx] !== false;
...
function addPlanToTasks() {
  ...
  state.workoutPlan.days.forEach((day, idx) => {
    if (day.type === 'Rest') return; const td = new Date(wkDate); td.setDate(wkDate.getDate() + idx); td.setHours(7, 0, 0, 0);
...
window.addPlanToTasks = addPlanToTasks;
window.authError = authError;
window.bindJournal = bindJournal;
window.buildSuggestions = buildSuggestions;
window.closeDrawer = closeDrawer;
window.closeGoalDrawer = closeGoalDrawer;
window.closeModal = closeModal;
window.closeUserMenu = closeUserMenu;
window.completeTask = completeTask;
window.confirmDeleteAccount = confirmDeleteAccount;
window.countProgress = countProgress;
...
window.setGoalContentMode = setGoalContentMode;
window.setJMode = setJMode;
...
window.toggleWDay = toggleWDay;
window.userDataKey = userDataKey;
